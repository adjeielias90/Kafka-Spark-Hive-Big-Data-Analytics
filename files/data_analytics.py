...
# programming_query = spark.sql("SELECT text FROM tweets WHERE text rlike '.*(programming).*'")
olympics_query = spark.sql("SELECT text FROM tweets WHERE text rlike '.*(olympics).*'")

# programming = programming_query.toPandas()
big_data = big_data_query.toPandas()
olympics = olympics_query.toPandas()
...
olympics['sentiment'] = pd.Series()
olympics['sentiment'] = olympics['text'].apply(lambda tweet: get_sentiment(tweet))


# programming['sentiment'] = pd.Series(dtype=str)
# programming['sentiment'] = programming['text'].apply(lambda tweet: get_sentiment(tweet))


big_data['sentiment'] = pd.Series(dtype=str)
big_data['sentiment'] = big_data['text'].apply(lambda tweet: get_sentiment(tweet))
...
big_data.sentiment = big_data.sentiment.replace({'pos': 1, 'neg': 0})
...
big_data['sentiment'].plot(kind='hist', bins=10, figsize=(8,10), title="2021 Big Data Sentiments: Positive:1, Negative: 0")
plt.show()

# save results of our analyses to disk as csv files for further processing
olympics.to_csv(r'./analyzed_data/olympics.csv', index=True, header=True)
big_data.to_csv(r'./analyzed_data/big_data.csv', index=True, header=True)
